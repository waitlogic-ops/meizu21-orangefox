...
- 源码可从 vendor_boot 的 /lib/modules 加载模块，但需要显式启用对应选项；当前主要依靠原厂 first-stage init，需要进一步验证是否足够。
- 原厂 qseecomd 二进制含 vendor.sys.listeners.registered 属性名。

## 构建后必须处理/验证

- 检查原厂模块是否实际加载；若缺失，在设备树中启用按原厂清单的模块加载，不能采用 Pro 的全部模块列表。
- 检查 qseecomd 访问 ssd 分区的时序。参考项目等待节点再启动，Gatekeeper 等待 listeners.registered；当前早期启动策略需审查。
- KeyMint 启动前应使用已安装系统的版本和安全补丁属性。参考 TWRP 16 的 prepdecrypt 逻辑不能直接假定存在于 OrangeFox 14.1，需要在实际同步的源码中核查。
